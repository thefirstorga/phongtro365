...
function InvoiceForm({ bookingId }) {
    ...
    return (
        <div>
            ...
            {/* Form thêm hóa đơn */}
            {showForm && (
                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-30" onClick={() => setShowForm(false)}>
                    <div className="bg-white p-8 rounded-lg shadow-lg w-full md:max-w-lg lg:max-w-3xl" onClick={(e) => e.stopPropagation()}>
                        <form onSubmit={saveForm}>
                            ...
                            <input
                                ...
                                className="w-full p-3 border border-gray-300 rounded-lg mb-4"
                            />
                            <p className="text-xl font-bold mt-4">Photos</p>
                            <PhotoUploader addedPhotos={addedPhotos} setAddedPhotos={setAddedPhotos} />
                            <button type="submit" className="w-full primary text-white p-3 rounded-lg mt-6">Save</button>
                        </form>
                        <button
            ...
            <div className='space-y-3'>
                ...
                <button className="primary" onClick={() => setShowForm(true)}>Thêm hóa đơn</button>
            </div>
        </div>
    );
}
